export function initHeroMotion(signal: AbortSignal) {
 ...
 let reduced = preference.matches || Boolean((navigator as Navigator & {connection?: {saveData?: boolean}}).connection?.saveData);
 try { const saved = localStorage.getItem('cuantica-reduced'); if (saved !== null) reduced = saved === 'true'; } catch {}
 let state: 'idle' | 'loading' | 'playing' | 'reversing' | 'released' = scrollY > innerHeight / 2 ? 'released' : 'idle';
 ...
 const locked = () => state === 'loading' || state === 'playing' || state === 'reversing';
 const topOf = (el: HTMLElement) => scrollY + el.getBoundingClientRect().top;
 function setState(next: typeof state) { state = next; stage.dataset.motionState = next; }
 function show(video: HTMLVideoElement) { videos.forEach(v => v.classList.toggle('is-current', v === video)); }
 function load() {
   if (reduced || failed) return;
   videos.forEach(video => {
     if (!video.getAttribute('src')) { video.muted = true; video.src = video.dataset.src!; video.load(); }
   });
 }
 function labels(frame: number) {
   // Keep the approved text timing relative to the original complete sequence.
   const progress = (frame - first) / (192 - first) * .94;
   const opacity = String(1 - ease((progress - .15) / .3));
 ...
 function moveTo(y: number) {
   ...
   previousY = lockedY;
   if (Math.abs(scrollY - lockedY) > .5) window.scrollTo({top: lockedY, behavior: 'instant'});
 }
 function unlock() {
 ...
 function complete() {
   if (!locked()) return;
   active.pause();
   if (Number.isFinite(active.duration)) active.currentTime = (last - first) / fps;
 ...
 function tick() {
   ...
   const frame = direction === 'down' ? Math.min(last, first + elapsed) : Math.max(first, last - elapsed);
   stage.dataset.frame = String(Math.round(frame));
   labels(frame);
   // Both directions use the same path. Below frame 80 the page stays at the hero.
   moveTo(startY + (endY - startY) * ease((frame - release) / (last - release)));
 ...
 function begin(nextDirection: typeof direction) {
   ...
   active.play().then(() => {
     if (thisRun !== run || !locked()) { active.pause(); return; }
     show(active);
     setState(direction === 'down' ? 'playing' : 'reversing');
     tick();
   }).catch(() => { if (thisRun === run) fail(); });
 }
 function enabled() { return !reduced && !failed && !document.querySelector('dialog[open]'); }
 function intercept(event: Event, delta: number) {
   if (locked()) { event.preventDefault(); return; }
   if (!enabled() || delta === 0) return;
   if (delta > 0 && state === 'idle' && Math.abs(story.getBoundingClientRect().top) < 2) {
     event.preventDefault(); begin('down');
   } else if (delta < 0 && state === 'released' && shop.getBoundingClientRect().top >= -2 && scrollY > topOf(story) + 2) {
     event.preventDefault(); begin('up');
   }
 }
 window.addEventListener('wheel', e => {
   if (!e.ctrlKey) intercept(e, Math.abs(e.deltaY) >= Math.abs(e.deltaX) ? e.deltaY : 0);
 }, {passive: false, signal});
 window.addEventListener('touchstart', e => { touchY = e.touches[0]?.clientY ?? 0; }, {passive: true, signal});
 ...
 window.addEventListener('keydown', e => {
   if (e.key === 'Escape' && locked()) { run++; complete(); return; }
   const target = e.target as HTMLElement;
   if (target.closest('input,textarea,select,button,a,[contenteditable="true"]') || e.ctrlKey || e.metaKey || e.altKey) return;
   if (['ArrowDown', 'ArrowUp', 'PageDown', 'PageUp', ' ', 'Home', 'End'].includes(e.key)) {
     intercept(e, ['ArrowDown', 'PageDown', 'End'].includes(e.key) || (e.key === ' ' && !e.shiftKey) ? 1 : -1);
   }
 }, {signal});
 window.addEventListener('scroll', () => {
   if (locked()) { if (scrollY !== lockedY) window.scrollTo({top: lockedY, behavior: 'instant'}); return; }
   ...
   if (upward && enabled() && state === 'released' && shop.getBoundingClientRect().top >= 0 && scrollY > topOf(story) + 2) begin('up');
 }, {passive: true, signal});
 videos.forEach(video => {
   video.addEventListener('loadeddata', () => {
     if (!locked() && state === 'idle' && video === forward) show(forward);
   }, {signal});
   video.addEventListener('ended', () => { if (video === active) complete(); }, {signal});
   video.addEventListener('error', fail, {signal});
 });
 function motion(keepStage = false) {
   const onStage = story.getBoundingClientRect().top <= 0 && story.getBoundingClientRect().bottom > innerHeight * .5;
   run++;
   videos.forEach(v => v.pause());
   unlock();
   ...
   if (keepStage && onStage) { moveTo(topOf(story)); setState('idle'); forward.currentTime = 0; show(forward); labels(first); }
   load();
 }
 toggles.forEach(toggle => toggle.addEventListener('click', () => {
   reduced = !reduced;
   try { localStorage.setItem('cuantica-reduced', String(reduced)); } catch {}
   motion(true);
 }, {signal}));
 preference.addEventListener('change', e => { reduced = e.matches; motion(true); }, {signal});
 window.addEventListener('resize', () => { startY = topOf(story); endY = topOf(shop); }, {signal});
 document.addEventListener('visibilitychange', () => { if (document.hidden && locked()) { run++; complete(); } }, {signal});
 window.addEventListener('pagehide', () => { if (locked()) { run++; complete(); } }, {signal});
 signal.addEventListener('abort', () => { run++; videos.forEach(v => v.pause()); unlock(); }, {once: true});
 setState(state);
 labels(first);
 motion();
}
